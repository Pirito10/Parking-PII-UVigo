...
public class Parking {
    ...
    private TreeMap<Visit, Integer> ingresos;
    private final static int COST_BY_MINUTE = 3;
    private final static int COST_BY_KWH = 12;
    ...
    public void carEntry(Car car, String time, boolean chargerRequired) {
    ...
    public boolean savePayments(String filename) throws FileNotFoundException {
        ...
        Set<Visit> keys = ingresos.keySet();

        for (Visit v : keys) {
            fichero.println(v.getEntryTime() + ";" + v.getPlate() + ";" + ingresos.get(v));
        }
    ...
    public int computeTimeCost(String entryTime, String departureTime) {
        float hoursSpent = CarDB.intervalInHours(entryTime, departureTime);
        float minutesSpent = hoursSpent * 60;
        return (int) (minutesSpent * COST_BY_MINUTE);
    }

    public int computeEnergyCost(float consumedEnergy) {
        return (int) Math.ceil(consumedEnergy * COST_BY_KWH);
    }

    public void registerPayment(String plate, String entryTime, String departureTime, float consumedEnergy) {
        int cost = computeTimeCost(entryTime, departureTime) + computeEnergyCost(consumedEnergy);
        ingresos.put(new Visit(plate, entryTime), cost);
    }
    ...
    public HashSet<CarSpace> getBusyCarSpaces() {
        return this.busyCarSpaces;
    }
}