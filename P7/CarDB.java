...
public class CarDB implements PIIDigitizable {
    ...
    public void readCityCarsFile(String filename) throws FileNotFoundException, PIIFileArgumentNotFound {
    ...
    public boolean saveToBinary(String filename) throws IOException {
    ...
    public boolean savePowersToBinary(String filename) throws IOException {

        DataOutputStream dos = new DataOutputStream(new FileOutputStream(filename));

        for (Car car : cityCars.values()) {
            dos.writeInt(car.getTotalPower());
        }
    ...
    public void readBinaryCityCars(String filename) throws IOException {
    ...
    public byte[] readBinaryFile(String filename) throws IOException {
        ...
        byte[] bytesArray = fis.readAllBytes();
    ...
    public TreeMap<String, Car> streamToMap(byte[] bytesArray) throws IOException, ClassNotFoundException {

        ByteArrayInputStream bais = new ByteArrayInputStream(bytesArray);
        ObjectInputStream ois = new ObjectInputStream(bais);

        TreeMap<String, Car> mapa = new TreeMap<>();

        try {
            // noinspection InfiniteLoopStatement
            while (true) {
                Car car = (Car) ois.readObject();
                mapa.put(car.getPlate(), car);
            }
    ...
    public boolean saveSeparatedCarTypes(TreeMap<String, Car> tm, String prefix) throws IOException {

        ObjectOutputStream oos_cc = new ObjectOutputStream(new FileOutputStream(prefix + "_cc.dat"));
        ObjectOutputStream oos_ec = new ObjectOutputStream(new FileOutputStream(prefix + "_ec.dat"));
        ObjectOutputStream oos_hc = new ObjectOutputStream(new FileOutputStream(prefix + "_hc.dat"));
        ...
        for (String s : keys) {
            ...
            if (car instanceof CombustionCar) {
                ...
                oos_cc.writeObject(car);

            } else if (car instanceof ElectricCar) {

                oos_ec.writeObject(car);

            } else {
                oos_hc.writeObject(car);
            }
        }
